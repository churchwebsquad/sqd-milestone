...
async function main(){
  ...
  const tplFilter = process.argv[2]
  ...
  const { data: sections } = await sb.from('web_sections')
    ...
    .eq('content_template_id', tplFilter)
    .not('cowork_slot_values', 'is', null)
  for (const s of (sections as any[])) {
    console.log('---', slugById.get(s.web_page_id), '/', s.id.slice(0,8), '---')
    console.log('content_template_id:', s.content_template_id)
    console.log('cowork_slot_values:')
    console.log(JSON.stringify(s.cowork_slot_values, null, 2))
    console.log('field_values:')
    console.log(JSON.stringify(s.field_values, null, 2))
  }
}
