...
import { extractStrategy } from '../../../lib/webAgents'
import type { ExtractStrategyError } from '../../../lib/webAgents'
...
export function RoadmapWorkspace({ project, onChange }: Props) {
  ...
  const [agentError, setAgentError] = useState<ExtractStrategyError | null>(null)
...
function StageCard({ stage, currentStage }: { stage: StageDef; currentStage: WMRoadmapStage }) {
    ...
    <div className="mb-5 rounded-md border border-wm-danger/30 bg-wm-danger-bg p-4">
      <div className="flex items-start justify-between gap-3 mb-2">
        <div className="flex items-center gap-2">
          ...
          <p className="text-[13px] font-semibold text-wm-danger">Strategy extraction couldn't run</p>
        </div>
        <button
          type="button"
          onClick={onDismiss}
      ...
      {error.missing_sources && error.missing_sources.length > 0 && (
        <div className="mb-2">
