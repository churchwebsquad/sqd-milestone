      ...
      <div className="max-w-4xl mx-auto">
        ...
        <WMCard padding="loose" className="mb-6">
          ...
          <div className="space-y-5">
            ...
            {/* Sitemap proposal — folded inline once Stage 2 has run. */}
            {hasSitemap && stage2 && (
              <SitemapSections data={stage2} viewMode={view} />
            )}
...
// ── Sitemap sections (Stage 2 output, folded into the Roadmap card) ──

function SitemapSections({
  data, viewMode,
}: { data: Record<string, unknown>; viewMode: ViewMode }) {
  ...
  return (
    <div className="space-y-5 pt-2 border-t border-wm-border">
      <div className="flex items-center gap-2 text-[10px] uppercase tracking-widest font-bold text-wm-accent-strong">
        <Compass size={11} /> AI sitemap proposal
        {meta?.extracted_at && (
          <span className="text-wm-text-subtle font-normal normal-case">· {new Date(meta.extracted_at as string).toLocaleString()}</span>
        )}
      </div>
      ...
      {(navStrategy || voiceRegister || navPattern) && (
        <ExtractionSection title="Nav strategy">
      ...
      {navItems && navItems.length > 0 && (
        <ExtractionSection title="Primary navigation">
      ...
      {/* Phase summary */}
      {phaseSummary && (
        <ExtractionSection title="Phase summary">
          <KVGrid pairs={[
      ...
      {phase1Pages.length > 0 && (
        <ExtractionSection title={`Phase 1 pages · ${phase1Pages.length}`}>
      ...
      {phase2Pages.length > 0 && (
        <ExtractionSection title={`Phase 2 pages · ${phase2Pages.length}`}>
          <div className="space-y-2">
      ...
      {navOnlyPages.length > 0 && (
        <ExtractionSection title={`Nav-only · ${navOnlyPages.length}`}>
          <div className="space-y-2">
      ...
      {isStaff && absorbed && absorbed.length > 0 && (
        <ExtractionSection title="Absorbed content (nested or dropped)">
      ...
      {vocabulary && vocabulary.length > 0 && (
        <ExtractionSection title="Vocabulary decisions">
          <ul className="space-y-1.5">
            {vocabulary.map((v, i) => (
              <li key={i} className="text-[12px] text-wm-text-muted">
      ...
      {/* AEO keywords */}
      {isStaff && aeo && (
        <ExtractionSection title="AEO / GEO keyword targets">
          <KVGrid pairs={[
      ...
      {isStaff && csFlags && (
        ...
        <ExtractionSection title="CS flags">
          {Array.isArray(csFlags.hard_blockers) && csFlags.hard_blockers.length > 0 && (
      ...
      {/* Sources used (re-using same pattern as Stage 1) */}
      {isStaff && sources && (
        <ExtractionSection title="Sources used (Stage 2)">
          <KVGrid pairs={[
