...
export function ApprovedSitemapBanner({ projectId, churchName, showAllStatuses }: Props) {
  ...
  const config = {
    draft: {
      ...
      label:   'Content Strategy Review ·draft (not yet published)',
    },
    published: {
      classes: 'border-blue-300 bg-blue-50 text-blue-800',
      label:   'Content Strategy Review ·published, awaiting partner',
    },
    partner_reviewed: {
      classes: 'border-amber-300 bg-amber-50 text-amber-800',
      label:   'Content Strategy Review ·partner made edits, staff review pending',
    },
    approved: {
      classes: 'border-green-400 bg-green-50 text-green-800',
      label:   'Content Strategy Review ·approved, canonical for downstream tools',
    },
  }[review.status]
