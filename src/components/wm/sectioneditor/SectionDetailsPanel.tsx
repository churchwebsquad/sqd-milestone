  ...
  return (
    ...
    <aside className="w-full h-full flex flex-col bg-wm-bg-elevated min-h-0">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto px-4 py-4 space-y-5">
        ...
        {/* Content Model panel — strategist declares the content type
            for this section (Staff / Events / Values / etc.). Multiple
            sections can share one model so the dev handoff knows e.g.
            "these 3 event_grids all back the same Events CPT." */}
        {projectId && (
          <Section title="Content Model" defaultOpen={false}>
            <ContentModelPanel
              projectId={projectId}
...
// ── Content Model panel ──────────────────────────────────────────────
//
// Strategist-owned content modeling. Each section can declare which
// content model it belongs to (Staff / Events / Values / etc.); the
// dev handoff reads these declarations as ground-truth instead of
// inferring from bound templates.
...
import { supabase as wmSupabase } from '../../../lib/supabase'
...
function ContentModelPanel({
  ...
  const current = findModelForSection(models, sectionId)

  const load = useCallback(async () => {
    setLoading(true)
    const list = await loadContentModels(wmSupabase, projectId)
    setModels(list)
    setLoading(false)
  }, [projectId])
  ...
  const handleAttach = async (modelId: string) => {
    ...
    if (current && current.id !== modelId) {
      await disconnectSectionFromModel(wmSupabase, projectId, current.id, sectionId)
    }
    await connectSectionToModel(wmSupabase, projectId, modelId, sectionId)
    await load()
    setBusy(false)
  }
  ...
  const handleCreate = async () => {
    ...
    // Detach from any existing model first — a section belongs to one
    // model at a time.
    if (current) {
      await disconnectSectionFromModel(wmSupabase, projectId, current.id, sectionId)
    }
    await upsertContentModel(wmSupabase, projectId, model)
    await load()
    setDraftName('')
    setCreating(false)
    setBusy(false)
  ...
  if (loading) {
    return <p className="text-[11.5px] text-wm-text-subtle">Loading content models…</p>
  }
 ...
 *  / re-type fields. Persists on each row change. */
function ContentModelSchemaEditor({
  ...
  const save = async () => {
    if (!dirty) return
    setSaving(true)
    await upsertContentModel(wmSupabase, projectId, {
      ...model,
      schema:     draft,
