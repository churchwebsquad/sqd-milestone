...
CREATE TABLE IF NOT EXISTS strategy_srp_auto_jobs (
...
CREATE INDEX IF NOT EXISTS idx_srp_auto_jobs_week
  ON strategy_srp_auto_jobs (week_start DESC);

CREATE INDEX IF NOT EXISTS idx_srp_auto_jobs_member
  ON strategy_srp_auto_jobs (member);

-- Auto-update updated_at
CREATE OR REPLACE FUNCTION strategy_srp_auto_jobs_set_updated_at()
RETURNS TRIGGER LANGUAGE plpgsql AS $$
BEGIN NEW.updated_at = now(); RETURN NEW; END;
$$;

DROP TRIGGER IF EXISTS trg_srp_auto_jobs_updated_at ON strategy_srp_auto_jobs;
CREATE TRIGGER trg_srp_auto_jobs_updated_at
  BEFORE UPDATE ON strategy_srp_auto_jobs
  FOR EACH ROW EXECUTE FUNCTION strategy_srp_auto_jobs_set_updated_at();
