...
export default async function handler(req: any, res: any) {
  ...
  const token = process.env.CLICKUP_API_TOKEN ?? process.env.CLICKUP_MILESTONE_API_TOKEN ?? process.env.CLICKUP_STRATEGY_MILESTONE_TOKEN
  if (!token) return res.status(500).json({ error: 'No ClickUp token set' })
